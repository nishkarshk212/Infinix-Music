...
from Infinix import config, logger, userbot
...
class MongoDB:
    def __init__(self):
        ...
        self.downloads = self.db.downloads  # For saving downloaded songs info
    ...
    async def get_admins(self, chat_id: int, reload: bool = False) -> list[int]:
        from Infinix.helpers._admins import reload_admins
    ...
    async def get_downloaded(self, song_id: str) -> dict | None:
        return await self.downloads.find_one({"_id": song_id})

    async def add_downloaded(self, song_id: str, file_path: str, video: bool = False, downloaded_at: float = None):
        await self.downloads.update_one(
            {"_id": song_id},
            {
                "$set": {
                    "file_path": file_path,
                    "video": video,
                    "downloaded_at": downloaded_at or time(),
                    "last_used": time()
                }
            },
            upsert=True
    ...
    async def update_last_used(self, song_id: str):
        await self.downloads.update_one(
            {"_id": song_id},
            {"$set": {"last_used": time()}}
        )

    async def cleanup_old_downloads(self, days_old: int = 7):
        cutoff = time() - (days_old * 86400)
        await self.downloads.delete_many({"last_used": {"$lt": cutoff}})
